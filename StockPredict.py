...
if predict:
    ...
    # Define the order of the seasonal components (P, D, Q, S)
    P, D, Q, S = 1, 1, 1, 12  # Example values for yearly seasonality (monthly data)

    # Create and fit the SARIMA model
    sarima_model = sm.tsa.SARIMAX(data['test_df']['Adj Close'], order=(p, d, q), seasonal_order=(P, D, Q, S))
    sarima_result = sarima_model.fit()

    # Make predictions
    sarima_forecast = sarima_result.predict(start=data['test_df'].index[0], end=data['test_df'].index[-1],
                                            dynamic=False)

    # Scaling the SARIMA forecast from full prices to 0 to 1 float values (if needed)
    sarima_forecast = scaler.fit_transform(np.expand_dims(data['test_df']['Adj Close'].values, axis=1))
    ...
    arima_sarima = (arima_forecast + sarima_forecast) / 2
    sarima_lstm = (lstm_pred + sarima_forecast) / 2
    ...
    plt.plot(data['test_df'].index, y_test_actual, label='Actual Price', color="black")
    plt.plot(data['test_df'].index,arima_sarima, color="blue", label=f"Ensemble of ARIMA/GRU")
